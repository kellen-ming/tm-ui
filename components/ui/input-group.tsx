...
const InputGroupContext = React.createContext<InputGroupContext>({
  mode: 'dark'
})

type InputGroupProps = React.ComponentProps<"div"> & InputGroupContext;
...
const inputGroupVariants = cva(`
  ...
  `,
  {
    variants: {
      mode: {
        light: `
          text-text-quarterary-gray-default 
          bg-bgc-secondary-gray-default
          border-border-secondary-gray-default
          has-[[data-slot=input-group-control]:focus-visible]:border-border-tertiary-black-default
        `,
        dark: `
          text-text-primary-gray-default
          bg-bgc-secondary-black-default
          border-border-tertiary-black-default
          has-[[data-slot=input-group-control]:focus-visible]:border-border-tertiary-black-hover
        `,
      }
    }
  }
...
function InputGroup({ className, mode = 'dark', ...props }: InputGroupProps) {

  return (
    <InputGroupContext.Provider value={{ mode }}>
      <div
    ...
    variants: {
      align: {
        ...
        "block-start":
          ...
          //  "order-first w-full justify-start px-3 pt-3 [.border-b]:pb-3 group-has-[_[data-slot=input-group-control]]/input-group:pt-2.5",
        "block-end":
          "order-last w-full justify-start px-3 pb-3 [.border-t]:pt-3 group-has-[>input]/input-group:pb-2.5",
          // "order-last w-full justify-start px-3 pb-3 [.border-t]:pt-3 group-has-[_[data-slot=input-group-control]]/input-group:pb-2.5",
      },
    },
    defaultVariants: {
      align: "inline-start",
  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        //  (e.currentTarget.parentElement?.querySelector("[data-slot=input-group-control]") as HTMLElement | null)?.focus()
      }}
      {...props}
    />
  )
...
function InputGroupInput(props: InputProps) {
  const { className, clearable = false, ...rest} = props
  const { mode } = React.useContext(InputGroupContext)
